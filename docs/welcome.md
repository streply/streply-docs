---
sidebar_position: 1
slug: /
title: Welcome!
---

# Welcome to Streply docs!

Streply is a bug and log tracker, supporting the most popular frameworks.

:star: Created by developers, for developers!

## Table of contents

- [PHP](category/php)
  - [Quick start](php/quick-start)
  - [Configuration](php/configuration)
  - [Setting user data](php/user-data)
  - [Performance](php/performance)
  - [Capture events](category/capture-events)
      - [Errors](php/capture-events/errors)
      - [Logs](php/capture-events/logs)
      - [Activities](php/capture-events/activities)
  - [Frameworks](category/frameworks)
    - [Monolog](php/frameworks/monolog)
    - [Laravel](php/frameworks/laravel)
    - [Symfony](php/frameworks/symfony)
    - [Magento](php/frameworks/magento)
- [JavaScript](category/javascript)
  - [Quick start](javascript/quick-start)
  - [Configuration](javascript/configuration)
  - [Capture events](category/capture-events-1)
    - [Errors](javascript/capture-events/errors)
    - [Logs](javascript/capture-events/logs)
    - [Activities](javascript/capture-events/activities)
  - [Frameworks](category/frameworks-1)
    - [Vue.js](javascript/frameworks/vue)
    - [React](javascript/frameworks/react)
- [Logs formatting](/logs-formatting)
- [Product](category/product)
  - [Issues](product/issues)
  - [Logs](product/logs)
  - [Performance](product/performance)
  - [Alerts](product/alerts)