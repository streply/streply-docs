---
sidebar_position: 1
slug: /
title: Welcome!
---

# Welcome to Streply docs!

Streply is a bug and log tracker, supporting the most popular frameworks.

:star: Created by developers, for developers!

## Table of contents

- [Capture events](capture-events)
- [PHP](php)
  - [Quick start](php/quick-start)
  - [Configuration](php/configuration)
  - [Performance](php/performance)
  - [User data](php/user-data)
  - [CRON](php/cron)
  - [Capture events](php/capture-events)
      - [Errors](php/capture-events/errors)
      - [Logs](php/capture-events/logs)
      - [Activities](php/capture-events/activities)
  - [Frameworks](php/frameworks)
    - [Monolog](php/frameworks/monolog)
    - [Laravel](php/frameworks/laravel)
    - [Symfony](php/frameworks/symfony)
    - [Magento](php/frameworks/magento)
- [JavaScript](js)
  - [Quick start](js/quick-start)
  - [Configuration](js/configuration)
  - [Capture events](js/capture-events)
    - [Errors](js/capture-events/errors)
    - [Logs](js/capture-events/logs)
    - [Activities](js/capture-events/activities)
  - [Frameworks](js/frameworks)
    - [Vue.js](js/frameworks/vue)
    - [React](js/frameworks/react)
- [Python](python)
  - [Quick start](python/quick-start)
  - [Configuration](python/configuration)
  - [Capture events](python/capture-events)
    - [Errors](python/capture-events/errors)
    - [Logs](python/capture-events/logs)
    - [Activities](python/capture-events/activities)
- [Logs formatting](/logs-formatting)
- [Product](product)
  - [Issues](product/issues)
  - [Logs](product/logs)
  - [Monitors](product/monitors)
    - [Get started](product/monitors/get-started)
    - [Monitor](product/monitors/monitor)
    - [Incidents](product/monitors/incidents)
    - [Customizing monitors](product/monitors/customizing-monitors)
      - [Check frequency](product/monitors/customizing-monitors/check-frequency)
      - [Request timeout](product/monitors/customizing-monitors/request-timeout)
      - [Request setting](product/monitors/customizing-monitors/request-setting)
  - [Cron](product/cron)
  - [Performance](product/performance)
  - [Alerts](product/alerts)
    - [Issue](product/alerts/issue) 
    - [Search criteria](product/alerts/search)
    - [Errors number](product/alerts/errors-number) 